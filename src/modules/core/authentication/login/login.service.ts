  ...
  static loginGoogle = async (data: LoginGoogleDto) => {
    ...
    try {
      ...
      user = await db.transaction(async () => {
        const service = await UserService.createWithoutPassword({
          email,
          role: "STUDENT",
        });
        const user = service.data;

        await ProfileService.create({ ...createData }, user.id);
        return user;
      });
    } finally {
      if (!user) throw AuthErrors.unauthorized;
...
class LoginUtils {
  static createPayload = async (user: UserService["data"]) => {
    ...
    let profile: ProfileAttributes | null = null;
    try {
      const service = await ProfileService.getById(user.id);
      profile = ProfileResponseFull.parse(service.data);
    } catch {}
